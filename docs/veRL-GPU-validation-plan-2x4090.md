...
| TQ returns padded tensors while veRL expects nested | `openagora_verl` auto-patches `transfer_queue._TQClient.async_get_data` |
...
# 6. Install OpenAgora adapters (registers ArenaAgentLoop and patches TransferQueue)
...
The launcher script already sets:
...
**Solution**: `openagora_verl` monkey-patches `transfer_queue._TQClient.async_get_data()` at import time:
- `prompts`, `responses` → forced nested
- `attention_mask` → forced padded
- other variable-length fields → auto-detected
- fixed-length / scalar fields → padded / tensor

See `python/openagora-verl/src/openagora_verl/_patch_tq.py`.
...
- **OpenAgora-side change**: `python/openagora-verl/src/openagora_verl/_patch_tq.py` patches TransferQueue automatically.
