...
OpenAgora provides a clean TransferQueue adapter (`openagora_verl.transfer_queue`)
that converts the padded tensors returned by TransferQueue into the nested/padded
hybrid layout veRL expects. Enable it explicitly before starting the sync trainer:
...
This is the recommended path for production training.
...
| TransferQueue / replay | `openagora_verl.install_transfer_queue_backend()` | Multi-GPU / production |
