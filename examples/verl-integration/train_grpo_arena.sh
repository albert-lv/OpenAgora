...
#
# By default it uses the legacy RayPPOTrainer entry point (train_grpo_arena.py)
# which calls verl.trainer.main_ppo. For production deployments with
# TransferQueue, set TRAINER_ENTRY_POINT=./train_grpo_arena_sync.py.
#
# Prerequisites:
...
# Choose the trainer entry point.
#   - train_grpo_arena.py      -> verl.trainer.main_ppo (RayPPOTrainer)
#   - train_grpo_arena_sync.py -> verl.trainer.main_ppo_sync (TransferQueue-based)
TRAINER_ENTRY_POINT="${TRAINER_ENTRY_POINT:-$(dirname "$0")/train_grpo_arena.py}"
