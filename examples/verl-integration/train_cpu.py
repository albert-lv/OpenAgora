...
# Ray init (optional — only used when running inside the Docker Compose stack)
...
if RAY_ADDRESS:
    try:
        ...
        logger.warning("Ray not installed; skipping Ray init (fine for standalone CPU training)")
else:
    logger.info("No RAY_ADDRESS set; skipping Ray init (standalone CPU mode)")
...
def ppo_update(
    ...
    logger.info(f"    fwd={t1-t0:.1f}s loss={t2-t1:.1f}s bwd={t3-t2:.1f}s step={t4-t3:.1f}s")
