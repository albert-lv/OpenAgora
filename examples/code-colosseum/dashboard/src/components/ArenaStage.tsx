  ...
  useEffect(() => {
    ...
    setLiveDuel((prev) => {
      ...
      if (latest.type === 'agent_completed') {
        ...
        agent.estimated_cost_usd = (latest.estimated_cost_usd as number) ?? agent.estimated_cost_usd
      }
...
function AgentCard({ agent }: { agent: Duel['agent_a'] }) {
  ...
  return (
    <div className="bg-slate-900 rounded-lg border border-slate-700 p-4">
      ...
      {agent.rollout_id && (
        <div className="mb-2 text-xs text-slate-500 font-mono truncate" title={agent.rollout_id}>
          rollout: {agent.rollout_id}
        </div>
      )}
      <div className="mb-2 text-sm space-x-3">
        ...
        {agent.estimated_cost_usd !== undefined && agent.estimated_cost_usd > 0 && (
          <span className="text-slate-400">
            Cost:{' '}
            <span className="font-mono text-slate-200">
