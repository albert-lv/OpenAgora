...
def detect_problem(messages: list) -> str:
    ...
    if re.search(r"def\s+is_valid\s*\(", text):
        return "is_valid"
    ...
    if "common prefix" in lower:
        return "longest_common_prefix"
    if "valid parentheses" in lower or "parentheses" in lower:
        return "is_valid"

    return "two_sum"


def _is_variants_disabled() -> bool:
    """Parse DISABLE_VARIANTS env var, treating '0'/'false' as disabled."""
    value = os.environ.get("DISABLE_VARIANTS", "").lower()
...
def pick_solution(problem: str, seed: int | None, temperature: float) -> str:
    ...
    By default the demo returns a mix of correct and intentionally buggy
    variants so that GRPO group advantages are non-degenerate and the training
    curves show real learning.  Set ``DISABLE_VARIANTS=1`` to always return the
    canonical correct solution.
    """
    if _is_variants_disabled():
    ...
    # If the caller did not supply a seed, use a random one so variants are
    # still sampled.  Arena proxy only injects seed when sampling.seed is set.
    ...
    weights = [correct_weight] + [
        (1.0 - correct_weight) / max(1, len(variants) - 1)
    ] * (len(variants) - 1)
