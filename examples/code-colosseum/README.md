...
│   ├── roman-to-integer-strict/    # strict validation + subtractive rules
│   ├── busy-periods/               # interval sweep-line
│   ├── merge-sorted-logs/          # multi-source merge + stable ordering
│   ├── number-to-words/            # English words up to one trillion
│   ├── expression-evaluator/       # arithmetic parser with unary ops
│   ├── json-path-select/           # nested JSON traversal + wildcards
│   ├── sparse-matrix-mul/          # sparse matrix-vector multiplication
│   └── command-router/             # path routing with params + catch-all
├── agent/                          # sandbox agent + mock LLM
├── backend/                        # FastAPI orchestrator
├── dashboard/                      # React dashboard
├── training/                       # PPO/GRPO trainer skeleton
├── scripts/                        # dataset generator + HumanEval importer
