...
def _backend_reachable(url: str) -> bool:
    """Best-effort check that an OpenAI-compatible LLM backend is alive."""
    try:
...
def run_one(instance_id: str, use_llm: bool, no_fallback: bool, endpoint: str, first_build: bool) -> dict:
    ...
    if use_llm:
        ...
        backend = env["ARENA_LLM_BACKEND"]
        if not _backend_reachable(backend):
            print(
                f"[{instance_id}] Warning: LLM backend {backend} is not reachable. "
                "Make sure Ollama or vLLM is running."
            )
