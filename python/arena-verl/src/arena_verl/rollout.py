...
Usage::
    ...
    from arena_verl.rollout import ArenaRollout
    ...
    rollout = ArenaRollout(
        ...
        arena_agent_image="arena-agent-minimal:latest",
        arena_llm_backend="http://localhost:8000/v1",
        arena_verify_command="pytest -k regression",
    )
...
tokenized and returned as ``responses`` in the output ``DataProto``.
    ...
    Args:
        ...
        arena_verify_command: Shell command to run for verification.
    ...
    def __init__(
        ...
        *,
        ...
        arena_verify_command: Optional[str] = None,
        ...
        **kwargs,
        ...
        self._arena_endpoint = arena_endpoint or os.environ.get("ARENA_ENDPOINT", "localhost:9090")
        self._agent_image = arena_agent_image or os.environ.get("ARENA_AGENT_IMAGE", "arena-agent-minimal:latest")
        self._llm_backend = arena_llm_backend or os.environ.get("ARENA_LLM_BACKEND", "http://localhost:8000/v1")
        self._verify_command = arena_verify_command or os.environ.get("ARENA_VERIFY_COMMAND", "true")
        ...
        if hasattr(model_config, "tokenizer") and model_config.tokenizer is not None:
            ...
            try:
                ...
                logger.warning("Failed to load tokenizer from %s: %s", getattr(model_config, "path", None), exc)

        if self._tokenizer is None:
            raise RuntimeError("ArenaRollout requires a tokenizer. Pass it via model_config.tokenizer or model_config.path.")
        ...
        logger.info(
            ...
            self._arena_endpoint, self._agent_image, self._llm_backend,
        )
    ...
    def generate_sequences(self, prompts: DataProto) -> DataProto:
        ...
        with ThreadPoolExecutor(max_workers=self._max_concurrent) as executor:
            ...
            for task_idx, (orig_idx, text) in enumerate(tasks):
                ...
                futures[executor.submit(
                    self._run_one, orig_idx, text, sampling_cfg, response_length, eos_token_id, task_idx
                )] = key
            ...
            for future in futures:
                ...
                try:
                    ...
                    logger.exception("Rollout %d sample %d failed: %s", orig_idx, task_idx, exc)
        ...
        if n > 1:
            ...
            position_ids_expanded = position_ids.repeat_interleave(n, dim=0)
        else:
        ...
        for i in range(batch_size):
            for j in range(n):
                ...
                expanded_status.append(results[idx]["status"] if idx < len(results) else "unknown")
    ...
    def _run_one(
        ...
        # Extract logprobs.
        log_probs = self._extract_logprobs(trajectory, len(response_ids))
    ...
    def _extract_response_text(self, trajectory: list[dict[str, Any]]) -> str:
        """Concatenate assistant messages from trajectory steps."""
        texts = []
    ...
    def _extract_logprobs(
        ...
        """Extract per-token logprobs from trajectory."""
        logprobs: list[float] = []
        import json
        for step in trajectory:
            resp = step.get("response") or {}
