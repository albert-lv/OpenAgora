...
for mod_name in list(sys.modules.keys()):
    if mod_name.startswith("verl") or mod_name.startswith("arena_verl"):
...
class FakeTokenizer:
    ...
    def encode(self, text, add_special_tokens=False):
        return [int(x) for x in text.split() if x.isdigit() or (x.startswith("-") and x[1:].isdigit())]
...
class TestArenaRollout:
    ...
    def test_generate_sequences(self, mock_client_cls, fake_tokenizer, mock_client):
        ...
        position_ids = torch.arange(prompt_length).unsqueeze(0).expand(batch_size, prompt_length)
    ...
    @patch("arena_sdk.client.ArenaClient")
    def test_generate_sequences_n_greater_than_one(self, mock_client_cls, fake_tokenizer, mock_client):
        mock_client_cls.return_value = mock_client
