...
class ArenaAgentLoop(AgentLoopBase):  # type: ignore[valid-type,misc]
    ...
    @staticmethod
    def _resolve_logger(value: Any) -> TrainingLogger:
        """Resolve a logger from Hydra config or a live instance."""
        if value is None:
            return NoOpLogger()
        if isinstance(value, TrainingLogger):
            return value
        # Hydra may instantiate via _target_ or pass a DictConfig.
        if hasattr(value, "log_rollout") and callable(value.log_rollout):
            return value  # type: ignore[return-value]
        # Allow env-based fallback to console logger for quick debugging.
        if _get_env("ARENA_CONSOLE_LOGGER", "0") == "1":
            from openagora_verl.logger import ConsoleLogger
    ...
    async def run(
    ...
    ) -> AgentLoopOutput:  # type: ignore[return]
        ...
        # veRL metadata: global_steps is passed via kwargs by the AgentLoopManager.
        # Providing min/max_global_steps enables veRL's staleness tracking and
        # off-policy detection in the trainer. For single-turn Arena rollouts the
        # policy version doesn't change between request and response, so both are
        # equal to the current global_steps.
        ...
        self._logger.log_rollout(
            ...
            extra={
                ...
                "create_seconds": rollout_create_time,
                "trajectory_steps": len(trajectory),
            },
        )
    ...
    def _apply_chat_template(self, messages: list[dict[str, Any]]) -> str:
        ...
        processing_class = (
            self._processor if self._processor is not None else self._tokenizer
        )
            ...
            try:
                ...
                if msgs:
                    ...
                    if last_role in ("user", "system"):
                        return (
                            "assistant"  # LLM is generating a response to user/system
                        )
